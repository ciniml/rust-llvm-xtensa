...
static unsigned getPCRelReloc(unsigned Kind) {
  switch (Kind) {
  case FK_Data_4:
    return ELF::R_Xtensa_CALL;
  case Xtensa::fixup_xtensa_brlo:
    return ELF::R_Xtensa_BRANCH;
  case Xtensa::fixup_xtensa_brhi:
    return ELF::R_Xtensa_BRANCH;
  case Xtensa::fixup_xtensa_jal:
    return ELF::R_Xtensa_JAL;
  case Xtensa::fixup_xtensa_call:
    return ELF::R_Xtensa_CALL;
  }
...
static unsigned getTLSLEReloc(unsigned Kind) {
  switch (Kind) {
  case FK_Data_4:
    return ELF::R_Xtensa_TLS_TPREL32;
  }
  llvm_unreachable("Unsupported absolute address");
}
...
static unsigned getPLTReloc(unsigned Kind) {
  switch (Kind) {
  case Xtensa::fixup_xtensa_call_plt:
    return ELF::R_Xtensa_CALL_PLT;
  }
  llvm_unreachable("Unsupported absolute address");
}
